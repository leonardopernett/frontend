...
export class ReporteUsuarioComponent implements OnInit {
 ...
 cargando = false
  ...
  async totalpaginas(){
    return new Promise<void>((resolve, reject) => {
      this.reportsApi.getUsuarioTotal(this.selectedPcrc.id_dp_pcrc)
        .pipe(first())
        .subscribe(
          (data:any) => {
          
            this.totalpage = data.total_usuario;
  ...
  limpiar()
    {
      this.router.navigate(['/app/redirect/usuario'])
    }
    ...
    if (!!this.selectedCliente && this.selectedCliente.cliente != 'Cualquiera') {
      ...
      if (!!this.selectedPcrc && this.selectedPcrc.pcrc != 'Cualquiera') {
        ...
        filter1 = { value: this.selectedPcrc.id_dp_pcrc.toString(), filter: 'pcrc' }
          ...
          this.reportsApi.getUsuario(this.selectedPcrc.id_dp_pcrc,this.page).subscribe(
 ...
 sendData(){
  ...
  if (this.selectedCategory==null) {
    ...
    this.reportsApi.getReportUsuarioPcrc( this.selectedPcrc.id_dp_pcrc,this.pagexportar)
    .subscribe(
      response => {
