...
@Component({
	selector: 'app-control-cambio',
	templateUrl: './control-cambios.component.html',
	styleUrls: ['./control-cambios.component.css'],
	encapsulation:ViewEncapsulation.None
})

export class ControlCambioComponent {
	...
	constructor(
		...
		private route: ActivatedRoute
			...
			this.idarticulo = this.route.snapshot.paramMap.get('idarticulo');
		 }

	ngOnInit() {
		
		this.select(this.idarticulo)
		this.onSelectionChange(this.id,2)

	  }

	  select(idarticulo){

		this.articlesApi.selectcambio(idarticulo).subscribe((data:any)=>{

			this.options=data.map(item=>{
				return {
					name: item.date_old+" - "+item.title_old+" - "+item.username, value: item.id
				}
			})
		})
	  ...
	  onSelectionChange(event,tipo) {
        
		let idcambio

		if(tipo==1){
			idcambio=event.target.value
		}else if(tipo==2){
			idcambio=event
			this.seleccionado=event
		}
		
		this.articlesApi.mostrarcambio(idcambio).subscribe(data=>{

			this.titulo_actual=data[0].title_new
			this.contenido_actual=data[0].content_new
			this.titulo_antiguo=data[0].title_old
			this.contenido_antiguo=data[0].content_old
			this.contenido_cambios=this.prueba(data[0].content_old, data[0].content_new)
			this.titulo_cambios=data = this.prueba(data[0].title_old, data[0].title_new)
		})

	  }

	prueba(originalHTML, newHTML){
