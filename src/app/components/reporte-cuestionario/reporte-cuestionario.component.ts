...
@Component({
  selector: 'app-reporte-cuestionario',
  templateUrl: './reporte-cuestionario.component.html',
  styleUrls: ['./reporte-cuestionario.component.css']
})
...
export class ReporteCuestionarioComponent implements OnInit {
  ...
  async totalpaginas(){
    return new Promise<void>((resolve, reject) => {
      this.reportsApi.getCuestionarioTotal(this.initialDateParam, this.finalDateParam, this.selectedPcrc.id_dp_pcrc)
        .pipe(first())
        .subscribe(
          (data:any) => {
           
            this.totalpage = data.total_cuestionarios;
  ...
  async totalpaginascategoria(){
    return new Promise<void>((resolve, reject) => {
      this.reportsApi.getCuestionarioCategoriaTotal(this.initialDateParam, this.finalDateParam, this.selectedCategory.id)
        .pipe(first())
        .subscribe(
          (data:any) => {
            this.totalpage = data.total_cuestionarios;
   ...
   this.elem.nativeElement.querySelector('.btn-primary2').classList.add('show')
    ...
    if (!!this.selectedCliente && this.selectedCliente.cliente != 'Cualquiera') {
      ...
      if (this.selectedCategory==null) {
        ...
        this.reportsApi.resultadoCuestionario(this.initialDateParam,this.finalDateParam,this.selectedPcrc.id_dp_pcrc,this.page).subscribe(data=>{
      ...
      if(!!this.selectedCategory && this.selectedCategory.name != 'Cualquiera'){
        ...
        this.reportsApi.resultadocategoriaCuestionario(this.initialDateParam,this.finalDateParam,this.selectedCategory.id,this.page).subscribe(data=>{
 ...
 limpiar(){
  this.router.navigate(['/app/redirect/cuestionario'])
 }
...
this.reportsApi.getReportCuestionarioPcrc(this.initialDateParam, this.finalDateParam, this.selectedPcrc.id_dp_pcrc,this.pagexportar)
.subscribe(
  response => {
...
if(!!this.selectedCategory && this.selectedCategory.name != 'Cualquiera'){
  ...
  this.reportsApi.getReportCuestionarioCategoria(this.initialDateParam, this.finalDateParam, this.selectedCategory.id,this.pagexportar)
.subscribe(
  response => {
    const blob = new Blob([response], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
    const file = window.URL.createObjectURL(blob);
