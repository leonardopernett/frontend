...
export class ReporteCambiosComponent implements OnInit {
  ...
  async totalpaginas(){
    return new Promise<void>((resolve, reject) => {
      this.reportsApi.getCambioTotal(this.initialDateParam, this.finalDateParam, this.selectedPcrc.id_dp_pcrc)
        .pipe(first())
        .subscribe(
          (data:any) => {
          
            this.totalpage = data.total_cambio;
  ...
  async totalpaginascategoria(){
    return new Promise<void>((resolve, reject) => {
      this.reportsApi.getCambioCategoriaTotal(this.initialDateParam, this.finalDateParam, this.selectedCategory.id)
        .pipe(first())
        .subscribe(
          (data:any) => {
            this.totalpage = data.total_cambio;
  ...
  limpiar(){
    this.router.navigate(['/app/redirect/cambios'])
  }
  ...
  getFilters() {
    ...
    document.getElementById('btn-primary').classList.add('show')
    document.getElementById('btn-primary2').classList.add('show')
      ...
      if (!!this.selectedPcrc && this.selectedPcrc.pcrc != 'Cualquiera') {
        ...
        filter1 = { value: this.selectedPcrc.id_dp_pcrc.toString(), filter: 'pcrc' }
         ...
         if(this.selectedCategory==null){
          ...
          this.reportsApi.getCambioPcrc(this.initialDateParam,this.finalDateParam,this.selectedPcrc.id_dp_pcrc, this.page).subscribe(
            (data:any) =>{
               this.data2= data
        ...
        if (!!this.selectedCategory && this.selectedCategory.name != 'Cualquiera') {
          ...
          this.reportsApi.getCambiocategoria(this.initialDateParam,this.finalDateParam,this.selectedCategory.id, this.page).subscribe(
            (data:any) =>{
               this.data2= data
 ...
 sendData(){
  ...
  this.reportsApi.getReportCambioPcrc(this.initialDateParam, this.finalDateParam, this.selectedPcrc.id_dp_pcrc,this.pagexportar)
  .subscribe(
    response => {
  ...
  if(!!this.selectedCategory && this.selectedCategory.name != 'Cualquiera'){
    ...
    this.reportsApi.getReportCambioCategoria(this.initialDateParam, this.finalDateParam, this.selectedCategory.id,this.pagexportar)
  .subscribe(
    response => {
      const blob = new Blob([response], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
      const file = window.URL.createObjectURL(blob);
